...
class GO:
    ...
    def findBestMove(self, piece_type):
        ...
        if (total_pieces == 0 and piece_type == 1) or (total_pieces == 1 and piece_type == 2 and middle_empty):
            return 2, 2
        else:
            best_alpha_beta_moves, alpha_beta_max_value = go.alphaBeta(2, piece_type)

        if not best_alpha_beta_moves:
            return ['PASS']
        else:
